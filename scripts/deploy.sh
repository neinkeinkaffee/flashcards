...
kubectl_exec postgres "DB_EXISTS=$(psql -U postgres -tAc \"SELECT 1 FROM pg_database WHERE datname='flashcards'\") && echo $DB_EXISTS" || kubectl_exec postgres createdb -U postgres flashcards
